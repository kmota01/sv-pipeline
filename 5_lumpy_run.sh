...
#Extract mean & standard deviation (stdev) values from lib1.histo (Those values are used to run Lumpy)
a=$(cat $output_dir/${sample}.lib1.histo | awk '{print $1}' | awk -F ':' '{print $2}')
mean=$(echo $a | awk '{printf "%d\n",$1}')
b=$(cat $output_dir/${sample}.lib1.histo | awk '{print $2}' | awk -F ':' '{print $2}')
